declare global {
  interface Window { __builderBuyBridgeMounted?: boolean }
}
...
export function initBuilderBuyBridge() {
  ...
  async function post(url: string, body: any) {
    ...
    if (!res.ok) {
      const msg = (parsed && parsed.error) || txt || `HTTP ${res.status} ${res.statusText}` || 'Checkout error';
      console.error('Checkout server error:', res.status, res.statusText, txt, parsed);
      throw new Error(msg);
    }
    ...
    try {
      const json = parsed ?? (await (async () => { try { return await (res as any).json(); } catch { return undefined; } })());
      if (json?.url) {
  ...
  function onClick(e: Event) {
    ...
    if (btn.hasAttribute('data-bundle')) {
      ...
      // Bundle checkout does not require per-item size selection
